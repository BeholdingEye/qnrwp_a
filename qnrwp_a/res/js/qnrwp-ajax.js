...
QNRWP.Ajax.request = function (data, mode, callType) {
    try {
        ...
        dataString += 'qnrwp_ajax_cookie=' + QNR.getCookieValue("qnrwp_ajax_cookie");
        var xhr=new XMLHttpRequest();
        // qnrwp_ajax_handler is defined in functions.php; QNRWP_JS_Global defined in script enqueue
        if (mode == "sync") {
            // QNR.AjaxSync(url, mode, request, contentTypeL, customHeaderL)
            var aR = QNR.AjaxSync(QNRWP_JS_Global.Ajax.url + "?action=qnrwp_ajax_handler", 
                                callType, dataString,
